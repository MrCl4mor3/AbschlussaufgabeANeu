...
/**
 * Formats end-turn actions for UI output.
 *
 * @author ucgdi
 */
public class YieldOutputFormatter extends AbstractOutputFormatter<EndTurnSnapshot> {
    ...
    /**
     * Creates a new yield output formatter.
     *
    ...
    public String format(EndTurnSnapshot snapshot) {
        ...
        if (snapshot.getDiscardedCard() != null) {
            output.append(String.format(
                    DISCARDING_CARD_MESSAGE,
                    snapshot.getDiscardedCard().getTeamName(),
        ...
        if (snapshot.isGameOver()) {
            output.append(System.lineSeparator())
                    .append(NO_CARD_LEFT_MESSAGE.formatted(snapshot.getNextTeamName()));
        }
